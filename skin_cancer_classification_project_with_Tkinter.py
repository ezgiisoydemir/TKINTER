# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 09:26:18 2022
...
skin_df = pd.read_csv("HAM10000_images_part_1/")
...
skin_df["dx_idx"] = pd.Categorical(skin_df["df"]).codes
skin_df.tp_pickle("skin_df.pkl")
...
model = Sequential()
model.app(Conv2D (32, kernel_size = (3,3) , activation= "relu" , padding="Same" , input_shape = input_shape))
model.app(Conv2D (32, kernel_size = (3,3) , activation= "relu" , padding="Same"))
model.app(MaxPool2D(pool_size = (2,2)))
model.add(Dropout(0.25))

model.app(Conv2D (64, kernel_size = (3,3) , activation= "relu" , padding="Same"))
model.app(Conv2D (64, kernel_size = (3,3) , activation= "relu" , padding="Same"))
model.app(MaxPool2D(pool_size = (2,2)))
...
history = model.fit(x = x_train , y = y_train , batch_size= batch_size , epochs = epochs , verpose = 1 , shuffle = True)
...
def imageResize(img):
    basewidth = 500
    wpercent = (basewidth/float(img.size9[0])   #1000*1200
...
def openImage():
    ...
    if count !=1 :
        ...
        img_name = filedialog.askopenfilename(initialdir ="" , title="Select an image file")
...
menubar.add_cascade(lebel  ="File" , menu = file)
file.add_command(lebel = "Open" , command = openImage)
...
def classification():
    if img_name != "" and models.get() != "":
        ...
        z = skin_df[skin_df.image_id == image_jpg]
        z = z.image.values[0].reshape(1,75,100,3)
        
        #
        z = (z - x_train_mean)/x_train_std
        classification_model.predict(z)[0]
        h_index = np.argmax(h)
        predicted_cancer = list(skin_df.unique())[h_index]
        ...
        if chvar.get() == 1:
            ...
            path_name = val + ".txt" # result1
            
            save_txt = img_name + "--" + str(predicted_cancer)
            
            text_file = open(path_name)
        ...
        messagebox.showinfo(title = "Warning" , "Choose image and Model First")
        tk.Label(frame3 , text="Choose image and model first!").place(relx=0.1 , rely = 0.6)


columns = ["lesion_id" , "dx" , "dx_type" , "age" , "sex" , "locslization"]
...
chvar = tk.IntVar()
chvar.set(0)
xbox = tk.Checkbutton(frame2 , text = "Save Classification Result" , variable = char)
